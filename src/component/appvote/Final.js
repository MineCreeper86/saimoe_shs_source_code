...
function Final() {
    ...
    const SubjectInput = (props) => {
        ...
        const parseSelected = (target) => {
            ...
            if (selected === target) {
                setSelected(0)
                document.getElementsByClassName("CopyrightInfo")[0].innerText = "";
        ...
        const children = props.option.characters.map((character) => (<td className=
                                                                         ...
                                                                         onClick={()=>{parseSelected(character.id)}}><img className={'CharacterAvatar'} src={'/avatars/'+character.image_src}/>{character.name_cn}</td>))
