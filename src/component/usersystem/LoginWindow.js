...
function LoginWindow(props) {
    ...
    const doForgetPassword = () => {setForgetPassword(true);};
    let schools = []
    let pending = false
    const isWechat = () => {
    ...
    if(state === 0) {
        ...
        return (
            <div className="LoginWindow">
                ...
                {state===1 && !forgetPassword && <p className="Tips">若2024年已注册可直接登录。<span onClick={doForgetPassword}>{forgetPassword?"":"忘记密码？"}</span></p>}
                {state===1 && forgetPassword && <p className="Tips">请使用去年注册时使用的邮箱向 feedback@shswafu2025.club 申请注销后重新注册</p>}
            </div>
        )
    }
}
